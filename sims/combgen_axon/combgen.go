...
// combgen runs combinatorial generalization
package main
...
var ParamSets = params.Sets{
	{Name: "Base", Desc: "these are the best params", Sheets: params.Sheets{
		"Network": &params.Sheet{
			...
			{Sel: "#InputToHidden1", Desc: "try stuff",
				Params: params.Params{
					"Prjn.WtScale.Abs": "1.2",
				}},
			{Sel: "Layer", Desc: "defaults",
				Params: params.Params{
					"Layer.Inhib.Layer.Gi":  "1",
					"Layer.Learn.AvgL.Gain": "2.5",
					"Layer.Act.Gbar.E":      "1.2",
					"Layer.Act.Gbar.L":      "0.2",
					"Layer.Act.Init.Decay":  "0.5", // 0.5 > 1 > 0
				}},
			{Sel: ".Back", Desc: "top-down back-projections MUST have lower relative weight scale, otherwise network hallucinates",
				Params: params.Params{
					"Prjn.WtScale.Rel": "0.2", // .2 > higher
				}},
			{Sel: ".Hidden", Desc: "std config it seems",
				Params: params.Params{
					"Layer.Inhib.Layer.Gi":    "1.1",
					"Layer.Inhib.ActAvg.Init": "0.04", // .04 > .05 overall
					"Layer.Inhib.Adapt.On":    "true",
				}},
			{Sel: "#Hidden1ToOutput", Desc: "no fail to localist output",
				Params: params.Params{
					"Prjn.Com.PFail": "0.0", // no fail
				}},
			{Sel: "#Output", Desc: "pool output inhib -- major benefit",
				Params: params.Params{
					"Layer.Inhib.Pool.On":  "true",
					"Layer.Inhib.Layer.On": "false", // major diffs
					"Layer.Inhib.Pool.Gi":  "0.8",   // .8 > .9
...
type Sim struct {
	...
	TrainEnv     CombEnv         `desc:"Training environment -- contains everything about iterating over input / output patterns over training"`
	TestEnv      CombEnv         `desc:"Testing environment -- manages iterating over testing"`
	...
	TrlOutName    string  `inactive:"+" desc:"name of pattern generated by network, based on closest matching patterns"`
	TrlPatErr     float64 `inactive:"+" desc:"current trial's closest pattern error -- proportion of mismatching pools"`
	TrlInvCor     float64 `inactive:"+" desc:"current trial's inverse correlation error against closest patterns"`
	TrlErr        float64 `inactive:"+" desc:"1 if trial was error, 0 if correct -- based on PatErr = 0"`
	TrlUnitErr    float64 `inactive:"+" desc:"current trial's unit-wise normalized error"`
	TrlCosDiff    float64 `inactive:"+" desc:"current trial's cosine difference"`
	EpcPatErr     float64 `inactive:"+" desc:"last epoch's average pattern error"`
	EpcInvCor     float64 `inactive:"+" desc:"last epoch's average invcor"`
	EpcUnitErr    float64 `inactive:"+" desc:"last epoch's average unit-wise normalized error"`
	...
	SumPatErr    float64                     `view:"-" inactive:"+" desc:"sum to increment as we go through epoch"`
	SumInvCor    float64                     `view:"-" inactive:"+" desc:"sum to increment as we go through epoch"`
...
func (ss *Sim) ConfigEnv() {
	...
	ss.TrainEnv.Config(Random, false, evec.Vec2i{7, 7}, 4, 500, 500)
	...
	ss.TestEnv.Defaults()
	ss.TestEnv.Config(Random, true, evec.Vec2i{7, 7}, 4, 500, 500)
	ss.TestEnv.Validate()
...
func (ss *Sim) ConfigNet(net *axon.Network) {
	net.InitName(net, "CombGen")
	inp := net.AddLayer4D("Input", 1, ss.TrainEnv.NPools, ss.TrainEnv.PatsSize.Y, ss.TrainEnv.PatsSize.X, emer.Input)
	hid1 := net.AddLayer2D("Hidden1", 20, 20, emer.Hidden)
	// hid2 := net.AddLayer4D("Hidden2", 2, 4, 3, 2, emer.Hidden)
	out := net.AddLayer4D("Output", 1, ss.TrainEnv.NPools, ss.TrainEnv.Output.Shp[2], ss.TrainEnv.Output.Shp[3], emer.Target)
...
func (ss *Sim) TrialStats(accum bool) {
	...
	if accum { // training
		ss.TrlPatErr, ss.TrlInvCor, ss.TrlOutName = ss.TrainEnv.OutputErr(ovt)
	} else {
		ss.TrlPatErr, ss.TrlInvCor, ss.TrlOutName = ss.TestEnv.OutputErr(ovt)
	}
	ss.TrlCosDiff = float64(out.CosDiff.Cos)
	ss.TrlUnitErr = out.PctUnitErr()
	if ss.TrlPatErr > 0 { // was UnitErr
		ss.TrlErr = 1
	} else {
		ss.TrlErr = 0
	}
	if accum {
		ss.SumErr += ss.TrlErr
		ss.SumPatErr += ss.TrlPatErr
		ss.SumInvCor += ss.TrlInvCor
...
func (ss *Sim) LogTrnEpc(dt *etable.Table) {
	...
	ss.EpcPatErr = ss.SumPatErr / nt
	ss.SumPatErr = 0
	ss.EpcInvCor = ss.SumInvCor / nt
	ss.SumInvCor = 0
	...
	dt.SetCellFloat("PatErr", row, ss.EpcPatErr)
	dt.SetCellFloat("InvCor", row, ss.EpcInvCor)
	...
	tst := ss.TstEpcLog
	if tst.Rows > 0 {
		cpc := []string{"PatErr", "InvCor", "UnitErr", "PctErr", "PctCor", "CosDiff"}
		for _, cn := range cpc {
...
func (ss *Sim) ConfigTrnEpcLog(dt *etable.Table) {
	...
	sch := etable.Schema{
		...
		{"PatErr", etensor.FLOAT64, nil, nil},
		{"InvCor", etensor.FLOAT64, nil, nil},
...
func (ss *Sim) ConfigTrnEpcPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "Axon Combinatorial Generalization Epoch Plot"
	plt.Params.XAxisCol = "Epoch"
	plt.SetTable(dt)
	// order of params: on, fixMin, min, fixMax, max
	plt.SetColParams("Run", eplot.Off, eplot.FixMin, 0, eplot.FloatMax, 0)
	plt.SetColParams("Epoch", eplot.Off, eplot.FixMin, 0, eplot.FloatMax, 0)
	plt.SetColParams("PatErr", eplot.On, eplot.FixMin, 0, eplot.FloatMax, 0)
	plt.SetColParams("InvCor", eplot.Off, eplot.FixMin, 0, eplot.FloatMax, 0)
...
func (ss *Sim) LogTstTrl(dt *etable.Table) {
	...
	dt.SetCellFloat("PatErr", row, ss.TrlPatErr)
	dt.SetCellFloat("InvCor", row, ss.TrlInvCor)
...
func (ss *Sim) ConfigTstTrlPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "Axon Combinatorial Generalization Test Trial Plot"
	...
	plt.SetColParams("PatErr", eplot.On, eplot.FixMin, 0, eplot.FloatMax, 0)
	plt.SetColParams("InvCor", eplot.On, eplot.FixMin, 0, eplot.FloatMax, 0)
	plt.SetColParams("UnitErr", eplot.Off, eplot.FixMin, 0, eplot.FloatMax, 0)
...
func (ss *Sim) LogTstEpc(dt *etable.Table) {
	...
	dt.SetCellFloat("Run", row, float64(ss.TrainEnv.Run.Cur))
	dt.SetCellFloat("Epoch", row, float64(epc))
	dt.SetCellFloat("UnitErr", row, agg.Mean(tix, "UnitErr")[0])
	dt.SetCellFloat("PatErr", row, agg.Mean(tix, "PatErr")[0])
	dt.SetCellFloat("InvCor", row, agg.Mean(tix, "InvCor")[0])
	...
	split.Agg(allsp, "PatErr", agg.AggMean)
	split.Agg(allsp, "InvCor", agg.AggMean)
...
func (ss *Sim) ConfigTstEpcPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "Axon Combinatorial Generalization Testing Epoch Plot"
	plt.Params.XAxisCol = "Epoch"
	plt.SetTable(dt)
	// order of params: on, fixMin, min, fixMax, max
	plt.SetColParams("Run", eplot.Off, eplot.FixMin, 0, eplot.FloatMax, 0)
	plt.SetColParams("Epoch", eplot.Off, eplot.FixMin, 0, eplot.FloatMax, 0)
	plt.SetColParams("PatErr", eplot.Off, eplot.FixMin, 0, eplot.FloatMax, 0)
	plt.SetColParams("InvCor", eplot.Off, eplot.FixMin, 0, eplot.FloatMax, 0)
	plt.SetColParams("UnitErr", eplot.Off, eplot.FixMin, 0, eplot.FloatMax, 0)
	plt.SetColParams("PctErr", eplot.On, eplot.FixMin, 0, eplot.FixMax, 1)  // default plot
	plt.SetColParams("PctCor", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1) // default plot
...
func (ss *Sim) ConfigTstCycPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "Axon Combinatorial Generalization Test Cycle Plot"
...
func (ss *Sim) LogRun(dt *etable.Table) {
	...
	dt.SetCellFloat("PatErr", row, agg.Mean(epcix, "PatErr")[0])
	dt.SetCellFloat("InvCor", row, agg.Mean(epcix, "InvCor")[0])
...
func (ss *Sim) ConfigRunPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "Axon Combinatorial Generalization Run Plot"
...
func (ss *Sim) ConfigGui() *gi.Window {
	...
	gi.SetAppAbout(`This tests for combinatorial generalization in Axon. See <a href="https://github.com/emer/emergent">emergent on GitHub</a>.</p>`)

	win := gi.NewMainWindow("combgen", "Axon Combinatorial Generalization", width, height)
	ss.Win = win
	...
	tbar.AddAction(gi.ActOpts{Label: "README", Icon: "file-markdown", Tooltip: "Opens your browser on the README file that contains instructions for how to run this model."}, win.This(),
		func(recv, send ki.Ki, sig int64, data interface{}) {
			gi.OpenURL("https://github.com/emer/axon/blob/master/examples/combgen/README.md")
		})
